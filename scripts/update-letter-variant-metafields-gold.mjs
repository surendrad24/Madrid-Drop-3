 ...
 * for all variants of the Letter Pendants Gold product.
 *
 * Usage:
 *   1. Copy .env.example to .env and fill in SHOPIFY_TOKEN
 *   2. node scripts/update-letter-variant-metafields-gold.mjs
 *
 * Or pass inline:
 *   SHOPIFY_TOKEN=shpat_xxx node scripts/update-letter-variant-metafields-gold.mjs
 */
...
const HANDLE   = 'letter-pendants-gold';
...
// Map letter → SKU  e.g. A → P027LPAG, B → P028LPBG … Z → P052LPZG
const LETTER_SKUS = Object.fromEntries(
  LETTERS.map((l, i) => {
    const num = String(i + 27).padStart(3, '0');
    return [l, `P${num}LP${l}G`];
  })
);

const SUBTITLE = '18CT GOLD VERMEIL // WHITE TOPAZ';
...
async function main() {
  ...
  for (const variant of product.variants) {
    ...
    const price = chain === 'no chain' ? '110.00' : '250.00';
